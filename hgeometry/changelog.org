...
- Using the new AsA mechanism for 'ccw'; this function now accepts
  both Point 2 r, as well as Point 2 r :+ extra. This means the ccw'
  function was removed as it is now no longer useful; just use ccw
  directly.
- Export the ToAPoint and ToPoint classes
- R1 to R4 instances for Point, so you can now also write ~p^._x~
  rather than ~p^.xCoord~ like, and also added a ~wCoord~ and
  ~wComponent~ lens to access the forth coordinat resp component of a
