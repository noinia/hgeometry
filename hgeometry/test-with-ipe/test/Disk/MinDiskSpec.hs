...
-- import qualified HGeometry.Disk.SmallestEnclosingBall.Naive as Naive
...
spec :: Spec
spec = testCases [osp|test-with-ipe/Disk/minDisk.ipe|]
...
testCases fp = do pts <- runIO $ readAllFrom =<< getDataFileName fp
...
groupByColor :: [IpeSymbol R :+ _] -> Map.Map (Maybe (IpeColor R)) (Set.Set (IpeSymbol R))
