...
_Vector :: Iso (Matrix n m r)                  (Matrix n m s)
               (Vector (n GHC.TypeLits.* m) r) (Vector (n GHC.TypeLits.* m) s)
_Vector = iso (\(Matrix v) -> v) Matrix
...
instance ( KnownNat n, KnownNat m)
  ...
  elements = reindexed quotRem (_Vector.components')
    where
      components' :: IndexedTraversal Int (Vector d r) (Vector d s) r s
      components' = components

  -- f i (Matrix v) = let m   = fromInteger $ natVal $ Proxy @m
  --                           in  (Matrix <$> components f i v)
