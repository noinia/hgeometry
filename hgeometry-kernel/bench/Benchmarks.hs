...
thePoints   :: Int -> IO ([Point2], [BoxedPoint2])
thePoints n = (\pts -> (pts, map pointFromPoint pts)) <$> randomPoints @Point2 n
...
main :: IO ()
main = do
  (!pts,!boxedPts) <- force <$> thePoints 100000
  defaultMain
    [ bgroup "sorting tests"
      [ bench "unboxed"           $ nf List.sort pts
      , bench "boxed "            $ nf List.sort boxedPts
